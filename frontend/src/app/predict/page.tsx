...
import { faFlask, faRotateLeft, faCircleExclamation, faDroplet, faScaleBalanced, faEye, faStar, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
    ...
    return (
      <div key={field}>
        <label className="flex items-center gap-1 text-sm font-medium text-gray-300 mb-1">
          ...
          {warn && <FontAwesomeIcon icon={faTriangleExclamation} className="w-3.5 h-3.5 text-amber-400" />}
        </label>
        <input
          type="text"
          inputMode="decimal"
    ...
    return (
      <div key={field}>
        <label className="flex items-center gap-1 text-sm font-medium text-gray-300 mb-1.5">
          {meta.label}
          {isBad && <FontAwesomeIcon icon={faTriangleExclamation} className="w-3.5 h-3.5 text-red-400" />}
        </label>
  ...
  const renderScoreField = (field: string) => {
    ...
    return (
      <div key={field}>
        <label className="flex items-center gap-1 text-sm font-medium text-gray-300 mb-1.5">
          {meta.label} (1-5)
          {isLow && <FontAwesomeIcon icon={faTriangleExclamation} className="w-3.5 h-3.5 text-red-400" />}
        </label>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="inline-flex items-center gap-2 px-3 py-1 glass rounded-full text-xs font-medium text-blue-400">
          <FontAwesomeIcon icon={faStar} className="w-3.5 h-3.5" /> Parameter Uji Susu
        </div>
        ...
        <form onSubmit={handleSubmit} className="xl:col-span-3 space-y-6 glass-dark p-6 rounded-2xl shadow-sm">
          <div className="space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-white/10">
              <FontAwesomeIcon icon={faScaleBalanced} className="w-5 h-5 text-blue-400" />
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-white/10">
              <FontAwesomeIcon icon={faEye} className="w-5 h-5 text-green-400" />
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-white/10">
              <FontAwesomeIcon icon={faDroplet} className="w-5 h-5 text-purple-400" />
          ...
          {error && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-start gap-3 p-3 glass border-red-500/20 rounded-xl">
              <FontAwesomeIcon icon={faCircleExclamation} className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />
              <div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading ? (
                ...
                <><FontAwesomeIcon icon={faFlask} className="w-5 h-5" /> Prediksi Sekarang</>
              )}
            </button>
            <button
              type="button"
            ...
            >
              <FontAwesomeIcon icon={faRotateLeft} className="w-4 h-4" /> Reset
            </button>
          </div>
        </form>
          ...
          <AnimatePresence mode="wait">
            ...
            {!result && !loading && (
              <motion.div key="placeholder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center h-full min-h-[400px] glass rounded-2xl border-2 border-dashed border-white/10 p-8 text-center">
                <FontAwesomeIcon icon={faFlask} className="w-16 h-16 text-blue-400 mb-4" />
